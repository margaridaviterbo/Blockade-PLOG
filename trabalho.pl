<<<<<<< HEAD
  ...
  /*play(board, player1, player2, player's turn, end of the game, type of game(menu), dificulty) */
  /*final condition*/play(_,_,_,_,1,1,_):-retract(pawn2(_)),retract(pawn1(_)),retract(pawn3(_)),retract(pawn4(_)),start(?).
  /*player against player*/play(L1,J1,J2,Turn,0,1,_):-J1=player(V1,H1),J2=player(V2,H2),display(L1,1,1),pawn1(Pawn1),pawn2(Pawn2),pawn3(Pawn3),pawn4(Pawn4),
  ((Turn==1,
  ...
  /*cpu against cpu*/play(Board,J1,J2,Turn,0,3,Dif):-J1=player(V1,H1),J2=player(V2,H2),display(Board,1,1),(Dif==1,movePawnRandomly(Board,Turn,StackBoard);Dif==2,smartMovementCPU(Board,Turn,StackBoard)),read(_),
  ...
  /*FAZER ISTO!!!!!!!!!!!!!player against cpu*/play(L1,J1,J2,Turn,0,2,Dif):-J1=player(V1,H1),J2=player(V2,H2),display(L1,1,1),(Dif==1,movePawnRandomly(Board,Turn,StackBoard);Dif==2,smartMovementCPU(Board,Turn,StackBoard)),read(_),
  ((Turn==1,
  	chooseStaringPiece(Turn,Pawn),
  	choosePositionToMove(Place),
    (Pawn==1,availablePositions(L1,Pawn1),availableList(List),member(Place,List),retract(availableList(_)),move(L1,Pawn1,Place,NewPawn,StackBoard,r),retract(pawn1(_)),asserta(pawn1(NewPawn)),((V1>0;H1>0),placeWall(V1,H1,StackBoard,NewBoard,NewVert,NewHor);NewBoard=StackBoard,NewVert is 0,NewHor is 0),checkWinners(End);
  	Pawn==2,availablePositions(L1,Pawn2),availableList(List),member(Place,List),retract(availableList(_)),move(L1,Pawn2,Place,NewPawn,StackBoard,r),retract(pawn2(_)),asserta(pawn2(NewPawn)),((V1>0;H1>0),placeWall(V1,H1,StackBoard,NewBoard,NewVert,NewHor);NewBoard=StackBoard,NewVert is 0,NewHor is 0),checkWinners(End)));fail.
    (Turn==2,(Dif==1,placeRandomWall(V1,H1,StackBoard,NewBoard,NewVert,NewHor);Dif==2,read(X),NewBoard=StackBoard,NewVert is 0,NewHor is 0),!,garbage_collect,play(NewBoard,player(NewVert,NewHor),J2,2,End,3,Dif),play(NewBoard,player(NewVert,NewHor),player(V2,H2),2,End,1,_))).
...
%?- calcboard(CalcBoard),replaceMatrix(CalcBoard,4,4,1,0,NewCalcBoard),asserta(calcBoard(NewCalcBoard)),board(Board),floodFill(Board,7,7,[C1,C2]),calcBoard(Final),write(Final).
?-start(?).
>>>>>>> 7bffa12a9d4190bc3b34afe6eb24ec4fc3402fbb
